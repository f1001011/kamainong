/**
 * @file 拉新奖励领取明细API
 * @description GET /api/admin/activities/invite/reward-claims
 * @depends 开发文档/02-数据层/02.4-后台API接口清单.md 第8.5节
 */
...
/**
 * GET /api/admin/activities/invite/reward-claims
 * 获取拉新奖励领取明细
 */
export async function GET(request: NextRequest) {
  return withAdminAuth(request, async (req) => {
    try {
      ...
      // 获取数据
      const result = await adminActivityService.getInviteRewardClaims(parsed.data);
      ...
      console.error('[InviteRewardClaims] 获取拉新奖励领取明细失败:', error);
      return errorResponse('INTERNAL_ERROR', '获取拉新奖励领取明细失败', 500);
    }
  });
}
