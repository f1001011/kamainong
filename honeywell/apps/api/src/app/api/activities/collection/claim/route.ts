/**
 * @file 领取连单奖励接口
 * @description 领取连单奖励活动的阶梯奖励
 * @depends 开发文档/02-数据层/02.3-前端API接口清单.md 第11.5节 - 领取连单奖励
...
/**
 * POST /api/activities/collection/claim
 * @description 领取连单奖励
 * @auth 需要登录
 ...
 * @error PREREQUISITE_NOT_MET - 前置条件未达成（prerequisite.isMet == false）
...
export async function POST(request: NextRequest): Promise<Response> {
  return withAuth(request, async (req, userId) => {
    ...
    // 领取奖励
    const result = await activityService.claimCollectionReward(userId, tier);
