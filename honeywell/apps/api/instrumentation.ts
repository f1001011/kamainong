...
export async function register() {
  console.log('[Instrumentation] register() 被调用, NEXT_RUNTIME:', process.env.NEXT_RUNTIME);
  
  // 仅在 Node.js 运行时（服务端）启动定时任务
  // 避免在 Edge Runtime 或客户端执行
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    console.log('[Instrumentation] Next.js 服务端启动，初始化定时任务...');
    
    try {
      // 动态导入避免在 Edge Runtime 中报错
      const { startAllJobs } = await import('./src/jobs');
      await startAllJobs();
