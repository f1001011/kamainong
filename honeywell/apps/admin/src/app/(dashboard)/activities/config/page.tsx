...
interface CollectionBonusConfig {
  prerequisiteDescription?: string;  // 前置条件描述（用户端显示）
  tiers: { products: string[]; reward: number; name?: string }[];  // name: 档位名称（用户端显示）
}
...
const ACTIVITY_ICONS: Record<string, React.ReactNode> = {
  ...
  WEEKLY_SALARY: <RiGiftLine size={28} />,
  PRIZE_POOL: <RiGiftLine size={28} />,
  SPIN_WHEEL: <RiGiftLine size={28} />,
  COMMUNITY: <RiGiftLine size={28} />,
};
...
const ACTIVITY_GRADIENTS: Record<string, string> = {
  ...
  WEEKLY_SALARY: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)',
  PRIZE_POOL: 'linear-gradient(135deg, #f5576c 0%, #ff6a88 100%)',
  SPIN_WHEEL: 'linear-gradient(135deg, #f7971e 0%, #ffd200 100%)',
  COMMUNITY: 'linear-gradient(135deg, #667eea 0%, #2f54eb 100%)',
};
...
export default function ActivityConfigPage() {
  ...
  const handleCloseDrawer = useCallback(() => {
    ...
    // 注意：form.resetFields() 移到 Drawer 的 afterOpenChange 回调中执行
    // 避免在 Form 组件卸载过程中调用导致警告
  }, []);
  
  // 抽屉动画完成后的回调
  const handleDrawerAfterOpenChange = useCallback((open: boolean) => {
    if (!open) {
      // 抽屉完全关闭后再重置表单，避免 "useForm not connected" 警告
      form.resetFields();
    }
  }, [form]);
  ...
  const renderCollectionBonusForm = () => (
    <>
      ...
      <Form.Item
        ...
        tooltip="用户端显示的活动参与前置条件说明"
        style={{ marginBottom: 24 }}
      >
        <Input
          placeholder="如: Recarga y compra un producto"
          style={{ width: '100%' }}
        />
      </Form.Item>
      ...
      <Form.List name="tiers">
        {(fields, { add, remove }) => (
          <>
              ...
              renderItem={(field, index) => {
                ...
                return (
                  <List.Item
                    ...
                    style={{ padding: '12px 0', borderBottom: '1px solid #f0f0f0' }}
                  >
                    <Row gutter={[16, 8]} style={{ width: '100%' }} align="top">
                      <Col span={2}>
                        <Tag color="purple" style={{ marginTop: 8 }}>档{index + 1}</Tag>
                      </Col>
                      <Col span={6}>
                        {/* 档位名称 */}
                        <Form.Item
                        ...
                        >
                          <Input
                            placeholder="如: Principiante"
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        {/* 产品编码输入 - 使用自定义转换处理数组类型 */}
                        <Form.Item
                          {...restField}
                          name={[name, 'products']}
                          label="所需产品"
                          ...
                          getValueFromEvent={(e: React.ChangeEvent<HTMLInputElement>) => {
                            // 将逗号分隔的字符串转换为数组
                            return e.target.value.split(',').map((s) => s.trim()).filter(Boolean);
                          }}
                      ...
                      <Col span={3} style={{ textAlign: 'right', paddingTop: 28 }}>
                        {fields.length > 1 && (
                          <Button
                            type="text"
                            danger
            ...
            <Button
              type="dashed"
              onClick={() => add({ products: [], reward: undefined, name: '' })}
              block
              icon={<RiAddLine size={16} />}
              style={{ marginTop: 16 }}
            >
  ...
  const renderConfigForm = () => {
    ...
    switch (currentActivity.code) {
      ...
      case 'WEEKLY_SALARY':
        return <Empty description="请前往「周薪配置」页面进行管理" />;
      case 'PRIZE_POOL':
        return <Empty description="请前往「奖池配置」页面进行管理" />;
      case 'SPIN_WHEEL':
        return <Empty description="请前往「转盘配置」页面进行管理" />;
      case 'COMMUNITY':
        return <Empty description="请前往「广场管理」页面进行管理" />;
      default:
