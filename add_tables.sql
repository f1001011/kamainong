...
CREATE TABLE `ntp_common_monthly_salary_config`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '月薪奖励配置表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ntp_common_agent_level_config`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '代理级别配置表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ntp_common_prize_pool_config`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '奖池配置表' ROW_FORMAT = Dynamic;
...
-- 商品表新增字段
ALTER TABLE `ntp_common_goods` ADD COLUMN `income_times_per_day` int(11) NOT NULL DEFAULT 1 COMMENT '每天收益次数' AFTER `day_red`;
ALTER TABLE `ntp_common_goods` ADD COLUMN `income_per_time` decimal(12, 2) NOT NULL DEFAULT 0.00 COMMENT '单次收益金额' AFTER `income_times_per_day`;

-- 用户表新增字段
ALTER TABLE `ntp_common_user` ADD COLUMN `agent_level` int(11) NOT NULL DEFAULT 0 COMMENT '代理级别 0-7' AFTER `level_vip`;
ALTER TABLE `ntp_common_user` ADD COLUMN `agent_level_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '代理级别名称' AFTER `agent_level`;
